...
public class UtilisateurControllerTest {
	...
	public void testerConnexionAvecSucces() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/")
			...
			.param("pseudo", "Abbey").param("motDePasse", "glows"))
			.andExpect(MockMvcResultMatchers.redirectedUrl("/avis"))
			.andExpect(status().isFound());
	}
	
	@Test
	public void testerInscription() throws Exception {

		MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders.get("/inscription");

		mockMvc.perform(requestBuilder).andExpect(view().name("inscriptionJoueurs")).andExpect(status().isOk());
	}
	...
	public void testerInscriptionAvecSucces() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/inscription")
			.accept(MediaType.TEXT_HTML)
			.param("pseudo", "Abbey").param("email", "a.g@hb.com").param("motDePasse", "glows").param("dateDeNaissance", "1999-10-18"))
	...
	public void testerDeconnexion() throws Exception {
		...
		mockMvc.perform(requestBuilder).andExpect(MockMvcResultMatchers.redirectedUrl("/")).andExpect(status().isFound());
	}
